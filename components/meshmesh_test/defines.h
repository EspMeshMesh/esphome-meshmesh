...
#define ERR_CHECK(X) if(X < 0) { STATE_LOGE("Response error"); CHANGE_STATE(mState, 99); } else
#define CHANGE_STATE(X,Y) mState = X; mSubState = Y; mLastTime = millis()
#define CHANGE_STATE_MSG(M,X,Y) STATE_LOGI(M); CHANGE_STATE(X,Y)
#define TIMEOUT_CHECK(X) if(millis() - mLastTime > X) { STATE_LOGE("Timeout reached"); CHANGE_STATE(mState, 99); }
